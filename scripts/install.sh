...
# the standard `skills` CLI. The installer owns distribution only; all hosts share the
...
cmd=(npx skills add "$SOURCE" --copy)
...
command -v npx >/dev/null 2>&1 || { echo "Error: npx is required to run Skills CLI" >&2; exit 1; }
exec "${cmd[@]}"
