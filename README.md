...
`bash scripts/install.sh` 默认以全局模式启动标准 `npx skills` 的交互目标选择，并使用
复制模式确保脚本资源随 skill 一起安装。Skills CLI 管理各宿主可发现的 skill 位置；
运行时状态始终共享在独立的 `~/.cli-bridge`，不会再把活跃会话状态写进 `~/.claude`。
